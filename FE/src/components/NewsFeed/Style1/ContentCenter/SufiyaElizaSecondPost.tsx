...
const SufiyaElizaSecondPost: FC<SufiyaElizaSecondPostInterFace> = ({post, shouldOpenComments, highlightCommentId, highlightReplyId, isShared}) => {
  const [localPost, setLocalPost] = useState(post);
  const [isDeleted, setIsDeleted] = useState(false);
  if (isDeleted) return null; 
  return (
    <div className="post-wrapper col-grid-box section-t-space d-block">
      <CommonUserHeading id="SufiyaElizaSecondPost" postUser={localPost} onPostUpdated={(updatedPost: Post) => setLocalPost(updatedPost)} onPostDeleted={() => setIsDeleted(true)} isShared={isShared} />
      <div className="post-details">
        ...
        {!isShared && (
          <>
            ...
            <CommonPostReact post={localPost} onReactionChange={(data) => setLocalPost({ ...localPost, reaction_counts: data.reaction_counts, total_reactions: data.total_count })} shouldOpenComments={shouldOpenComments} highlightCommentId={highlightCommentId} highlightReplyId={highlightReplyId} />
          </>
        )}
      </div>
    </div>
