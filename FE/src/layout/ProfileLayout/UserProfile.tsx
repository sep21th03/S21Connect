...
const UserProfile: FC<UserProfileInterFace> = ({
  ...
  const handleAddFriend = async () => {
    ...
    try {
      await axiosInstance.post(
        API_ENDPOINTS.FRIENDS.BASE + API_ENDPOINTS.FRIENDS.SEND(userProfile?.user.id)
      );
  ...
  const handleCancelRequest = async () => {
    ...
    try {
      await axiosInstance.delete(
        API_ENDPOINTS.FRIENDS.BASE + API_ENDPOINTS.FRIENDS.REJECT(userProfile?.user.id)
      );
      setFriendshipStatus("none");
    } catch (error) {
  ...
  const handleAcceptRequest = async () => {
    ...
    try {
      await axiosInstance.post(
        API_ENDPOINTS.FRIENDS.BASE + API_ENDPOINTS.FRIENDS.ACCEPT(userProfile?.user.id)
      );
  ...
  const handleUnfriend = async () => {
    ...
    try {
      await axiosInstance.delete(
        API_ENDPOINTS.FRIENDS.BASE + API_ENDPOINTS.FRIENDS.UNFRIEND(userProfile?.user.id)
      );
      setFriendshipStatus("none");
    } catch (error) {
  ...
  const renderFriendshipButton = () => {
    ...
    switch (friendshipStatus) {
      case "none":
        return (
          <button onClick={handleAddFriend} className="btn btn-solid" disabled={isLoading}>
            {isLoading ? "Đang xử lý..." : "Thêm bạn bè"}
          </button>
        );
      case "pending_sent":
        return (
          <button onClick={handleCancelRequest} className="btn btn-outline" disabled={isLoading}>
      ...
      case "pending_received":
        return (
          <div className="d-flex gap-2">
            <button onClick={handleAcceptRequest} className="btn btn-solid" disabled={isLoading}>
      ...
      case "accepted":
        return (
          <button onClick={handleUnfriend} className="btn btn-outline" disabled={isLoading}>
